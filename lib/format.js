...
/**
 * Canonical URL path for an article: `/{category}/{slug}`.
 * Falls back to the legacy `/article/{slug}` path when the joined category is
 * missing, so nothing ever renders a broken link.
 * @param {{ slug?: string, categories?: { slug?: string } } | null} article
 */
export function articleUrl(article) {
  if (!article || !article.slug) return '/';
  const categorySlug = article.categories?.slug?.trim();
  if (categorySlug) return `/${categorySlug}/${article.slug}`;
